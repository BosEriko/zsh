...
if [[ "$OS_TYPE" == "stm" ]]; then
  command -v obsidian >/dev/null 2>&1 || NIXPKGS_ALLOW_UNFREE=1 nix-env -iA nixpkgs.obsidian
fi

if [[ "$OS_TYPE" == "win" ]]; then
  if command -v winget.exe >/dev/null 2>&1; then
    winget.exe list -e --id Obsidian.Obsidian >/dev/null 2>&1 || winget.exe install -e --id Obsidian.Obsidian
  fi
fi
